...
export const pillReminderDeleteService = {
  ...
  async toggleReminder(id: number, isEnabled: boolean): Promise<boolean> {
    try {
      const result = await pillReminderRepository.toggleReminder(id, isEnabled);
  ...
  async deleteReminder(id: number): Promise<boolean> {
    try {
      const result = await pillReminderRepository.deleteReminder(id);
  ...
  async deleteAllReminders(): Promise<boolean> {
    try {
      const result = await pillReminderRepository.deleteAllReminders();
