import CustomAlert, { TCustomAlertButtons, TCustomModalType } from "@/components/organisms/CustomAlert";
...
const AlertProvider = memo(({ children }: { children: ReactNode }): React.JSX.Element => {
  const [visible, setVisible] = useState<boolean>(false)
  const [title, setTitle] = useState<string>('')
  const [message, setMessage] = useState<string>('')
  const [buttons, setButtons] = useState<TCustomAlertButtons[] | undefined>(undefined)
  const [modalType, setModalType] = useState<TCustomModalType>('default')
  const [checkboxLabel, setCheckboxLabel] = useState<string>('')
  const [onCheckboxChange, setOnCheckboxChange] = useState<(isChecked: boolean) => void>(() => { })
  const [onRequestClose, setOnRequestClose] = useState<(() => void) | undefined>(undefined)
